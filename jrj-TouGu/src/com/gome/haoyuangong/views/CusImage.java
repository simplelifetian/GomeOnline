...
//		myPaint.setColor(Color.rgb(0, 161, 234));  //Edit this to change progress arc color.
		myPaint.setColor(0xffde3031);  //Edit this to change progress arc color.
		myPaint.setStrokeWidth(stW);

		myFramePaint = new Paint();
		myFramePaint.setAntiAlias(true);
		myFramePaint.setColor(Color.TRANSPARENT);
		float startx = (float) (pix * 0);
		float endx = (float) (pix * 1);
		float starty = (float) (pix * 0);
		float endy = (float) (pix * 1);
		int temp = stW>>1;
		rect = new RectF(startx+temp, starty+temp, endx-temp, endy-temp);
	}
